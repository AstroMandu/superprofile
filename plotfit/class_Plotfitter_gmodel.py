...
class Gmodel:
    ...
    # === ADD: solve amplitudes/baseline via weighted LSQ ===
    def _solve_linear_A_B(self, V21, S21, V22, S22):
        """
        Solve for (A21, A22, B2?) given nonlinear params (V21, S21, V22, S22).
        If B2 is fixed (self.has_B2 == False), we solve only A21, A22 with y - B_fixed.
        Returns: (A21, A22, B2), model, loglike
        """
        G1 = np.exp(-0.5 * ((self.x - V21) / S21)**2)
        G2 = np.exp(-0.5 * ((self.x - V22) / S22)**2)
        w  = self.inv_e_y**2

        if self.has_B2:
            # Solve [A21, A22, B2]
            X  = np.column_stack((G1, G2, np.ones_like(self.x)))
            Xw = X * np.sqrt(w)[:, None]
            yw = self.y * np.sqrt(w)
            beta, *_ = np.linalg.lstsq(Xw, yw, rcond=None)
            A21, A22, B2 = beta
            model = A21*G1 + A22*G2 + B2
        else:
            # B fixed to B1; solve only [A21, A22]
            B2 = self.B1
            y0 = self.y - B2
        ...
        ll = -0.5 * np.sum(((self.y - model) * self.inv_e_y)**2)
        return (A21, A22, B2), model, ll


    # === ADD: nonlinear log-like that only optimizes [V21, V22, logS21, logS22] ===
    def log_prob_2G_nl(self, z):
        """
        z = [V21, V22, logS21, logS22] in unconstrained space.
        Amplitudes/Baseline are solved analytically each call.
        """
        V21  = z[0]
        V22  = z[1]
        S21  = np.exp(z[2])
        ...
        # keep your ordering rule
        if not self.test_2G_disp_order(S21, S22):
            return -np.inf

        (A21, A22, B2), model, ll = self._solve_linear_A_B(V21, S21, V22, S22)

        # (optional) soft penalty for negative amps; avoids killing exploration
        if A21 < 0:
            ll -= 1e6 * (abs(A21))
        if A22 < 0:
            ll -= 1e6 * (abs(A22))

        # (optional) your amplitude-cap prior, if you still want it
        # if not self.test_2G_ampl(A21, A22, B2):
        #     return -np.inf

        return ll


    # === ADD: minimize-friendly wrapper (like your log_prob_guess) ===
    def log_prob_2G_nl_guess(self, z):
        lp = self.log_prob_2G_nl(z)
        if ~np.isfinite(lp):
            return 1e20
        return -lp
    
    def _z_to_params(self, z):
        # interpret z in a fixed order per names_param (A, V, S, B)
        p = np.empty_like(z, dtype=np.float64)
        for i, name in enumerate(self.names_param):
            lo, hi = self.dict_bound[name]
            zi = z[i]
            if   name.startswith('A'):   p[i] = np.exp(zi)            # >0
            elif name.startswith('S'):   p[i] = np.exp(zi)            # >0
            elif name.startswith('V'):   p[i] = zi                    # unbounded
            elif name == 'B2':           p[i] = lo + (hi-lo)/(1+np.exp(-zi))  # [lo,hi]
            else:                         p[i] = zi
        return p
    
    def log_prob_2G_unconstrained(self, z):
        params = self._z_to_params(z)
        # optional: light checks (e.g., S22>S21)
        A21,A22 = params[self.argwhere_A21], params[self.argwhere_A22]
        S21,S22 = params[self.argwhere_S21], params[self.argwhere_S22]
        if not self.test_2G_disp_order(S21,S22): return -np.inf
        # NB: you can drop the heavy bound mapping now
        return self.log_L_2G(*params[[self.argwhere_A21, self.argwhere_A22,
                                    self.argwhere_V21 if self.has_V21 else None,
                                    self.argwhere_V22 if self.has_V22 else None,
                                    self.argwhere_S21, self.argwhere_S22,
                                    self.argwhere_B2  if self.has_B2  else None]])
